/**
 * Raw backend DTOs for the `/main/` endpoints.
 *
 * Every schema here mirrors the **wire shape** — field names, nullability,
 * and enum casing are whatever the backend emits. Parsers built with
 * `createEnvelopeParser` validate incoming JSON at the trust boundary
 * and hand a typed DTO to the feature-local mapper, which then converts
 * into the UI-facing domain model in `./api.ts`.
 *
 * When to edit this file:
 * - Backend adds, renames, or removes a field in an existing `/main/`
 *   response.
 * - A new `/main/` endpoint is introduced.
 *
 * Do NOT edit this file to rename a UI-facing field — that belongs in
 * `./api.ts`. Keeping DTO and domain types separate means backend renames
 * only touch the mapper, not every component.
 */
import { z } from 'zod';
