...
import type { SupplyItem } from '@/shared/types/api';
...
export function CollapsibleSection({ title, items, defaultOpen = false }: CollapsibleSectionProps) {
    ...
    <div>
      ...
      >
        ...
        <ChevronDown
          size={20}
          className={[
            'text-text-tertiary transition-transform duration-normal',
            open ? 'rotate-180' : '',
          ].join(' ')}
          aria-hidden="true"
        />
      ...
      {open && (
        <div className="mt-2 rounded-lg overflow-hidden animate-expand">
