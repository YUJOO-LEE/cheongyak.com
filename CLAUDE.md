...
- **Skeleton parity Playwright gate (Phase B-2b):** `e2e/skeleton-parity.spec.ts`
  ...
  the first real `<article>` — per-card parity is the right unit since
  ...
  + `getSubsForDate`) — any static fixture lands in the empty state 6
  days out of 7; the underlying skeleton-too-tall-for-empty-state
  issue is tracked as a follow-up in
  `docs/skeleton-parity-test-plan.md`. It is main-branch-only work —
  ...
  inside `<div hidden>` and atomically unwraps once ready —
...
`src/proxy.ts` matches `/api/backend/:path*` (Next 16 renamed `middleware.ts` → `proxy.ts` with a matching `proxy()` export — same execution model). It applies two zero-cost defenses against scraping / cost-spike attacks against the backend rewrite:
...
Pure helpers + tests live in `src/shared/lib/api-guard.ts` / `api-guard.test.ts`. Server-side fetches (RSC, sitemap, ISR revalidation) bypass `proxy.ts` because they call the backend directly via `API_BACKEND_URL`.
