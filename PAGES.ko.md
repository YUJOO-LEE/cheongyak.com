...
- 단계 칩은 `PhaseChip`(텍스트 전용, 아이콘 없음)으로 렌더. `phases[]`의 각 phase(`'특별공급' | '일반공급 1순위' | '일반공급 2순위' | '당첨자 발표'`)는 announcement의 `status`와 무관하게 고정된 (bg, text) 토큰 페어를 사용 — 사용자가 어느 날짜에서 보든 같은 phase는 동일한 색으로 칠해진다. 4개 토큰 페어는 DESIGN.md §11.3.2 참조. `StatusChip`의 `label?` 오버라이드를 통해 phase를 표시하지 말 것 — color와 label이 서로 다른 의미를 갖던 문제로 인해 해당 prop은 제거됨. `phases.length > 1`이면 칩을 여러 개 렌더.
