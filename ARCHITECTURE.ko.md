...
| **CLS** | < 0.1 | 모든 이미지에 명시적 `width`/`height`; 스켈레톤 플레이스홀더가 최종 레이아웃과 일치 (형제 `*.skeleton.tsx` + 라우트 `loading.tsx`, `loading.test.tsx` RTL 게이트로 고정 — `docs/skeleton-parity-test-plan.md` 참조); 레이아웃 변경 없는 폰트 스왑 |
