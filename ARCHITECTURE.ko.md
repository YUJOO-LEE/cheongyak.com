...
| **CLS** | < 0.1 | 모든 이미지에 명시적 `width`/`height`; 스켈레톤 플레이스홀더가 최종 레이아웃과 일치 (형제 `*.skeleton.tsx` + 라우트 `loading.tsx`, 매 PR 에서 `loading.test.tsx` RTL 게이트로 고정하고, `/listings` · `/listings/[id]` 는 main 전용 Playwright 바운딩 박스 게이트 `e2e/skeleton-parity.spec.ts` 로 추가 고정 — `docs/skeleton-parity-test-plan.md` 참조); 레이아웃 변경 없는 폰트 스왑 |
