...
**통합 검색:** 네비게이션 검색 아이콘 또는 `⌘K` 단축키로 오버레이 실행. `GET /api/search` (Next.js rewrite alias, 백엔드 `/apt-sales/search` 로 프록시; 청약 단지명만)에 연결되어 있고 최근 검색 기록은 localStorage 에 저장. 뉴스 검색은 아직 연결되지 않음.
...
`src/proxy.ts` 가 `/api/backend/:path*` 를 매칭 (Next 16 에서 `middleware.ts` → `proxy.ts` + `proxy()` export 로 rename — 실행 모델 동일). 백엔드 rewrite 에 대한 스크래핑/비용 폭증 공격에 대비한 두 가지 무비용 방어를 적용:
...
순수 헬퍼와 테스트는 `src/shared/lib/api-guard.ts` / `api-guard.test.ts` 에 있음. 서버 측 fetch (RSC, sitemap, ISR revalidation) 는 `API_BACKEND_URL` 로 백엔드를 직접 호출하므로 `proxy.ts` 를 거치지 않음.
